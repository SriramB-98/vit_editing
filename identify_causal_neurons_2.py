...
#     opt = torch.optim.SGD(all_parameters, lr=hyperparams['lr'])
    ...
    misc_params = {'mode':'train', 'thresh':hyperparams['thresh'], 'flip':False}
    for hook_name in hook_names:
    ...
    for e in range(hyperparams['epochs']):
        ...
        for i, (imgs, masks, labels) in enumerate(dataloader):
            ...
            masked_logits = model(masked_imgs).detach()            
            masked_features = store_dict['features'].detach()

            # get original logits
            for hook_name in hook_names:
                orig_acts_dict[hook_name] = 0
            _ = model(normalizer(imgs))
            orig_features = store_dict['features']
            orig_logits = model.head(orig_features.detach())
            ...
            loss1 = F.cross_entropy(orig_logits, labels.to(device))
            loss2 = F.mse_loss(orig_features, masked_features) #(1-F.cosine_similarity(orig_features, masked_features)).mean()

            
            # reg_vec = torch.cat([torch.sigmoid(w).reshape(-1) for w in weights_param.values()])
            # reg = reg_vec.abs().mean() #make the weights sparse
            total_loss = loss1 + hyperparams['beta']*loss2# + hyperparams['lambda']*reg

            clean_intervened_acc = (orig_logits.argmax(-1).cpu() == labels).float().mean().item()
            masked_acc = (masked_logits.argmax(-1).cpu() == labels).float().mean().item()
            ...
            if misc_params['mode'] == 'train':
                ...
                if i % log_freq == 0:
                    wandb.log({'loss 1 (train)': loss1.detach().item(),
                            ...
                            'clean intervened acc (train)': clean_intervened_acc,
                            'masked acc (train)': masked_acc,
                ...
                metric_accum_dict['masked acc (val)'].append(masked_acc)
                metric_accum_dict['clean intervened acc (val)'].append(clean_intervened_acc)
        ...
        del imgs, masks, labels, masked_imgs, orig_logits
        del total_loss, loss1, loss2
...
if __name__ == '__main__':
    ...
    run = wandb.init(
            project=f"find-causal-neurons-2-{args.model}_{args.dataset}_{args.mask_type}_flip-{args.flip}_{hook_name_str}",
            config=hyperparams
            )
