...
class ImageFolder(DatasetFolder):
    ...
    def __init__(self, root, transform=None, target_transform=None,
                 loader=default_loader, label_mapping=None, masks=False):
        super(ImageFolder, self).__init__(root, loader, IMG_EXTENSIONS,
                                          transform=transform,
                                          target_transform=target_transform,
                                          label_mapping=label_mapping)
        self.imgs = self.samples
        if masks and 'val' in root:
            self.masks = [get_mask_path(img_path) for img_path, _ in self.imgs]
    ...
    def __getitem__(self, index):
        ...
        if self.masks:
            fg_mask = np.load(self.masks[index])
            fg_mask = np.tile(fg_mask[:, :, np.newaxis], [1, 1, 3]).astype('uint8')
            fg_mask = self.ToTensor(Image.fromarray(fg_mask*255))
            return img, fg_mask, label
        else:
            return img, label
